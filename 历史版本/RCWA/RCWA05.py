...
def slicer(thickness,k):
    ...
    n_gr=1.00027
    subtract_thickness=10*1e-6
    single_thickness=(thickness+subtract_thickness)/k
    # num=int(np.ceil(thickness/single_thickness))#应该向上取整
    d=np.ones((k,1))*single_thickness
    fill_factor=np.ones((k,1))
    fill_factor[:int(thickness/single_thickness),0]=0.5
    n_rd=np.ones((k,1))*n_rd
    n_gr=np.ones((k,1))*n_gr
    n_gr=n_gr.astype('complex')
    n_gr[int(thickness/single_thickness):,0]=1.4482+j*7.5367
...
n_rd=1.4482+j*7.5367#光栅脊部折射率
n_gr=n1#光栅槽部折射率
thickness=2*1e-6#光栅区域2的厚度
fill_factor=0.5#光栅的占空比
d,fill_factor,n_rd,n_gr=slicer(thickness,16)
#计算光栅介电常数矩阵E
# Nx=1024
...
for lam in wavelengh:
    ...
    for i in range(len(d)):
        [f,g,a,X]=iteration(n_rd[i,0],n_gr[i,0],d[i,0],k0,KX,m,f,g,fill_factor[i,0])
        ...
        inv_a_XL=inv_a_XL@a_inv@X#inv_a_XL矩阵明显过大了
        #最终得到系数f1,g1
...
plt.title("4μm周期占空比0.5槽深2μm矩形光栅衍射效率计算结果")
plt.xlabel("衍射级次")
plt.ylabel("衍射效率/η")
plt.legend()
plt.show()
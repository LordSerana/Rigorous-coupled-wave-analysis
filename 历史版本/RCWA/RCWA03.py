...
#对RCWA02文件进行数值稳定性改进，主要着力解决a矩阵条件数过大，求a的逆矩阵为病态问题的新的解决方法
#本脚本求解的为TE偏振态
#设定全局计算精度
getcontext().prec=16
...
def Rectangular(T,Amplitude,fill_factor,t):
    '''
    T:矩形函数的周期
    Amplitude:幅值
    fill_factor:占空比（光栅脊部的）
    '''
    #实际上就是定义了一个矩形光栅函数
    return Amplitude if t<=T*fill_factor else 0
...
def iteration(d,k0,kx,m,f,g,E):
    ...
    # a=a_b[:2*m+1,:2*m+1]
    # b=a_b[2*m+1:,:2*m+1]
    f_L=W@(np.eye(2*m+1)+X@b@LA.inv(a)@X)
    g_L=V@(np.eye(2*m+1)-X@b@LA.inv(a)@X)
    # f_L=W@(np.eye(2*m+1)+X@a)
    # g_L=V@(np.eye(2*m+1)-X@a)
    return f_L,g_L,a,X

def slicer(thickness,lam0):
    '''
    用于将光栅区域切片,切片厚度取为波长的1/12~1/20较为适宜
    '''
    single_thickness=50*lam0/15
...
#下列部分为光栅的系数
m=20#傅里叶谐波数，指的是1~m次,这一项影响的是计算精度
...
n_gr=3.8827+j*0.019626#光栅槽部折射率
thickness=2*1e-6#光栅区域2的厚度
...
wavelengh=np.linspace(0.6,0.7,t)
for lam in wavelengh:
    ...
    R_Coeffi=np.block([[np.eye(2*m+1)],[-j*Z1]])#(2m+1,4m+2)
    ...
    for i in range(len(d)):
        [f,g,a,X]=iteration(d[i,0],k0,KX,m,f,g,E)
        ...
        '''
        inv_a_XL=inv_a_XL@np.linalg.inv(a)@X#inv_a_XL矩阵明显过大了
        #最终得到系数f1,g1
    ...
    #试试高斯消去法
    T=np.linalg.inv(j*np.matmul(Z1,f)+g)
    T=np.matmul(T,(np.matmul(j*Z1,delta_i0)+n_delta_i0))
    R=np.matmul(f,T)-delta_i0
    T=np.matmul(inv_a_XL,T)

...
#对6号脚本进行了修正，修正了光栅矩阵的表达式，以及使用fft方法时，改进了介电常数的表达式
class Material:
    '''
    n:材料折射率，一般取复数
    '''
...
#需要写入的系数只有以下三行
pi=np.pi
grating=Rectangular(4*1e-6,2*1e-6,0.5,"Air","Al","Al")
# grating=Triangular(4*1e-6,39,1,"Air","Al","SiO2")
m=15#傅里叶谐波数，指的是1~m次,这一项影响的是计算精度
order=6
thetai=1e-5*pi/180#入射角
d,fill_factor,n_rd,n_gr=slicer(grating,100)

...
#本脚本求解的为TE偏振态
#本脚本仅求解反射波
#对RCWA_partial脚本进一步改进。前一个脚本已可以稳定求解反射波、透射波的振幅，该脚本对光栅类型增加，并将其集成为一个光栅类
#对光栅透射区的边界条件进行拓展
...
for lam in wavelengh:
    ...
    for k in range(2*m+1):
        if k0**2*n1**2>k_xi[k]**2:
            kz1[k,0]=np.sqrt(k0**2*n1**2-k_xi[k]**2)
        elif k0**2*n1**2<=k_xi[k]**2:
            kz1[k,0]=-j*np.sqrt(k_xi[k]**2-k0**2*n1**2)
    
    for k in range(2*m+1):
        if k0**2*n2**2>k_xi[k]**2:
            kz2[k,0]=np.sqrt(k0**2*n2**2-k_xi[k]**2)
        elif k0**2*n2**2<=k_xi[k]**2:
            kz2[k,0]=-j*np.sqrt(k_xi[k]**2-k0**2*n2**2)

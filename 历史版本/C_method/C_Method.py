...
def Compute(eps1,eps2):
    ...
    A=A.flatten()
    B1=Constant['n1']**2-A*A#beta1
    B1=B1.flatten()
    B2=Constant['n2']**2-A*A#beta2
    B2=B2.flatten()
    ...
    a_diff_vec=F_series_gen(Constant['diff_a'],nDim)
    ...
    #Assemble F-matrices
    b0=np.sqrt(B1[-m1])
    F_in,FRN,FRP=GenerateFFieldsChand(a,b0,nDim,m1,m2,real_Ray2_idx,real_Ray1_idx,SB1,SB2)
    #Assemble G-matrices
    ...
    #绘制衍射效率曲线
    etaR,etaT=Plot_intensity(RVec,real_Ray1_idx,real_Ray2_idx,B1,B2,m1,b0,nDim,True)
    return etaR,etaT
...
def F_series_gen(profile,nDim,cut_small=True):
    ...
    该函数返回-N/2~N/2级谐波
    '''
    fourier_coeffi=np.fft.fftshift(np.fft.fft(profile,axis=0)/profile.shape[0])
    tol=1e-10
    #减去系数中的小量
    if cut_small==True:
    ...
    return fourier_coeffi[q0-nDim:q0+nDim]
...
def GenerateFFieldsChand(profile,b0,nDim,m1,m2,real_Ray2_idx,real_Ray1_idx,SB1,SB2):
    LP_fun=np.exp(-1j*b0*profile)#define fourier transform argument for positive fields
    ...
    if len(real_Ray1_idx)!=0:
        ...
        for n in range(min_ray1,max_ray1+1):
            LPn_fun=np.exp(1j*SB1[n]*profile)
    ...
    if len(real_Ray2_idx)!=0:
        ...
        for k in range(min_ray2,max_ray2+1):
            LNn_fun=np.exp(-1j*SB2[k]*profile)
...
def Plot_intensity(RVec,real_Ray1_idx,real_Ray2_idx,B1,B2,m1,b0,nDim,plot):
    ...
    if plot:
        ...
        if etaT.all()==0:
            ...
            plt.plot(x,etaT,label='Transmission')
        plt.legend()
...
etaR,etaT=Compute(Constant['eps1'],Constant['eps2'])
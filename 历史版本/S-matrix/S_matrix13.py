...
def Set_Polatization(thetai,phi,wavelength,ninc,pTE,pTM):
    thetai=np.radians(thetai)
    phi=np.radians(phi)
    n=[0,0,1]
    k0=2*pi/wavelength
    kinc=k0*ninc*np.array([np.sin(thetai)*np.cos(phi),np.sin(thetai)*np.sin(phi),np.cos(thetai)])
    if thetai==0:
        aTE=[0,1,0]
    else:
        aTE=np.cross(n,kinc)/np.linalg.norm(np.cross(n,kinc))
    aTM=np.cross(kinc,aTE)/np.linalg.norm(np.cross(kinc,aTE))
    p=np.dot(pTE,aTE)+pTM*aTM
    p=p/np.linalg.norm(p)
    return p

# grating=Rectangular(4*1e-6,2*1e-6,0.5,"Air","Al","Al",1*1e-6)
grating=Triangular(4*1e-6,25,1,"Air","Al","Al",5*1e-6)
p=Set_Polatization(0,0,lam,1,1,0)
...
# VirtualLab_R=[0.0069921,0.019589,0.0039815,0.061496,0.020376,0.22451,0.15543,0.22451,0.020376,0.061496,0.0039815,0.019589,0.0069921]#Al,30°线偏振光，矩形
# VirtualLab_R=[0.0060481,0.013476,0.0063769,0.055142,0.021614,0.21037,0.22983,0.21037,0.021614,0.055142,0.0063769,0.013476,0.0060481]#Al,右旋圆偏振光，矩形
#右旋计算结果与左旋完全一样
...
plt.legend()
plt.show()
print(R_effi[int(order[0]+m):int(order[len(order)-1]+m+1)])
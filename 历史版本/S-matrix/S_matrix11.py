...
def grating_model(E,E_recip,k0,d):
    ...
    LAM=np.diag(np.sqrt(LAM2,dtype=complex))
...
def calcSubstrate():
    ...
    omega=np.block([[1j*kztrn*np.eye(2*m+1),np.zeros((2*m+1,2*m+1))],[np.zeros((2*m+1,2*m+1)),1j*kztrn*np.eye(2*m+1)]])
...
VirtualLab_R=[0.0079,0.025,0.0015,0.068,0.019,0.238,0.081,0.238,0.019,0.068,0.0015,0.025,0.0079]#Al

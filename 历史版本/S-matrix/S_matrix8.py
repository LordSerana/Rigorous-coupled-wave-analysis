...
#7.py的脚本可以正确的模拟出三角光栅的衍射效率趋势了，但是在数值大小上还是有差距
...
def calcSubstrate():
    kztrn=np.sqrt(grating.n2**2-KX@KX-KY@KY)
    Q=np.block([[KX@KY,np.eye(2*m+1)*grating.n2**2-KX@KX],[KY@KY-np.eye(2*m+1)*grating.n2**2,-KY@KX]])
    omega=np.block([[j*kztrn,np.zeros((2*m+1,2*m+1))],[np.zeros((2*m+1,2*m+1)),j*kztrn]])
    V=Q@np.linalg.inv(omega)
    A=np.eye(4*m+2)+np.linalg.inv(V)@V0
    B=np.eye(4*m+2)-np.linalg.inv(V)@V0
    X=np.block([[j*kztrn*k0*grating.substrate_thickness,np.zeros((2*m+1,2*m+1))],[np.zeros((2*m+1,2*m+1)),j*kztrn*k0*grating.substrate_thickness]])

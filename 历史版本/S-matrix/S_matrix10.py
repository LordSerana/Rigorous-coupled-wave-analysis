...
#修正了8.py的脚本中关于X矩阵的计算错误。对于矩形光栅衍射效率的计算已经更精确了，接下来需要改进的是三角光栅槽型的衍射效率
...
def grating_model(E,E_recip,k0,d):
    ...
    P=np.block([[-KX@E_inv@KY,KX@E_inv@KX-np.eye(2*m+1)],[np.eye(2*m+1)-KY@E_inv@KY,KY@E_inv@KX]])
...
def calcFreeSpace():
    P=Q=-np.block([[KX@KY,np.eye(2*m+1)-KX@KX],[KY@KY-np.eye(2*m+1),-KX@KY]])
    # Q=np.block([[KX@KY,np.eye(2*m+1)+KY@KY],[-KX@KX-np.eye(2*m+1),-KX@KY]])
    # Q=-Q
    omega2=P@Q
    LAM2,W0=np.linalg.eig(omega2)
    LAM=np.diag(np.sqrt(LAM2))
    # W0=np.eye(4*m+2)
    V0=Q@W0@np.linalg.inv(LAM)
    return W0,V0

def calcRefle():
    P=-np.block([[KX@KY,grating.n1**2*np.eye(2*m+1)-KX@KX],[KY@KY-grating.n1**2*np.eye(2*m+1),-KY@KX]])
...
def calcSubstrate():
    kztrn=np.sqrt(grating.n2**2-kinc[0]**2-kinc[1]**2)
    Q=np.block([[KX@KY,np.eye(2*m+1)*grating.n2**2-KX@KX],[KY@KY-np.eye(2*m+1)*grating.n2**2,-KY@KX]])
    omega=np.block([[j*kztrn*np.eye(2*m+1),np.zeros((2*m+1,2*m+1))],[np.zeros((2*m+1,2*m+1)),j*kztrn*np.eye(2*m+1)]])
    V=Q@np.linalg.inv(omega)
    A=np.eye(4*m+2)+np.linalg.inv(V)@V0
    B=np.eye(4*m+2)-np.linalg.inv(V)@V0
    X=np.exp(omega*k0*grating.substrate_thickness)
    X=np.diag(np.diag(X))
    D=A-X@B@np.linalg.inv(A)@X@B
    S11=S22=np.linalg.inv(D)@(X@B@np.linalg.inv(A)@X@A-B)
    S12=S21=np.linalg.inv(D)@X@(A-B@np.linalg.inv(A)@B)
...
def calck():
    ...
    for i in range(2*m+1):
        kzref[i,i]=np.conj(np.sqrt(grating.n1**2-KX[i,i]**2))
    # for i in range(2*m+1):
    #     if grating.n1**2>KX[i,i]**2:
    #         kzref[i,i]=np.sqrt(grating.n1**2-KX[i,i]**2)
    #     else:
    #         kzref[i,i]=-j*np.sqrt(KX[i,i]**2-grating.n1**2)
...
phi=1*1e-5*pi/180#yz平面内的入射角，锥形入射情况
...
# VirtualLab_T=[0,0,0,0.02,0,0.09,0.2123,0.09,0,0.02,0,0,0]
VirtualLab_R=[0.0079,0.025,0.0015,0.068,0.019,0.238,0.081,0.238,0.019,0.068,0.0015,0.025,0.0079]
# plt.plot(order,VirtualLab_T,label='VLab_T')

...
#5.py对三角光栅的仿真是比较准确的，不再修改。本脚本中，不再使用Li规则，考虑到TE模式应该不需要使用Li规则
...
def grating_model(E,E_recip,k0,d):
    ...
    P=np.block([[-KX@E_inv@KY,KX@E_inv@KX-np.eye(2*m+1)],[np.eye(2*m+1)-KY@KY@E_inv,KY@E_inv@KX]])
...
grating=Triangular(4*1e-6,31.6,1,"Air","Al","Si",10*1e-6)

...
import numpy as np
# from C_Method.Grating import Triangular,Sinusoidal
...
def Compute(Constant):
    ...
    a_diff_vec=F_series_gen(Constant['a_diff'],nDim-1)
    ...
    U,s,Vt=la.svd(MatBC,full_matrices=False)
    threshold=np.max(s)*np.max(MatBC.shape)*np.finfo(float).eps
    s_inv=np.zeros_like(s)
    mask=(s>threshold)
    s_inv[mask]=1.0/s[mask]
    s_inv=s_inv.reshape(-1,1)
    Amplitude=Vt.T@(s_inv*(U.T@VecBC))
...
def GenerateFField(Constant):
    ...
    beta1_m=Constant['beta1_m']
    beta2_m=Constant['beta2_m']
    vect1_p=Constant['vect1_p']
    vect2_n=Constant['vect2_n']
    nDim=Constant['n_Tr']
    #=====Fmn=====
    if len(Constant['n1_set'])!=0:
        Fmn=np.zeros((len(m_set),len(n1_set)),dtype=complex)
        for m in range(n1_set[0],n1_set[-1]+1,1):
            temp=np.exp(-1j*beta1_m[m-m_set[0]]*Constant['a'])
            Fourier_Fmn=F_series_gen(temp,nDim-1)
            q0=len(Fourier_Fmn)//2
            part1=np.conj(Fourier_Fmn[q0+m_set[0]-m:q0])
            part2=np.conj(Fourier_Fmn[q0:q0+m_set[-1]-m+1])
            Fmn[:,m-n1_set[0]]=np.concatenate([part1,part2])
    else:
    ...
    if len(Constant['n2_set'])!=0:
        ...
        for k in range(n2_set[0],n2_set[-1]+1,1):
            temp=np.exp(-1j*beta2_m[k]*Constant['a'])
            Fourier_Fmk=F_series_gen(temp,nDim)
            q0=len(Fourier_Fmk)//2
            for m in range(n1_set[0],n1_set[-1]+1):
                Fmk[m,k]=Fourier_Fmk[q0+m-k]
    else:
        Fmk=np.zeros((len(m_set),len(n2_set)))
    #=====F_in_0===========
    temp=np.exp(-1j*beta1_m[int((Constant['n_Tr']+1)/2)]*Constant['a'])
    F_in_0=F_series_gen(temp,Constant['n_Tr'])#正场系数
    q0=len(F_in_0)//2
...
def GenerateGField(a_mat,Constant,eig1_p,eig2_n):
    ...
    for m in range(len(m_set)):
        ...
        for s in range(len(m_set)):
            Gm0_s[s,0]=(a_mat[m,s]*alpha_m[s]+a_ms[m,s]*beta1_m[int((Constant['n_Tr']-1)/2)])*Fm0[s]
...
# Constant['a(x)'],Constant['a_diff(x)']=grating.profile()
a=grating.profile()
...
if TM!=0:
    Polarization='TM'
    etaR_TM,etaT_TM=Compute(Constant)
if TE!=0:
    Polarization='TE'
    etaR_TE,etaT_TE=Compute(Constant)
if TM==0:
...
######################################################################
start_order=-6
[print(f"{start_order+i} {val}") for i,val in enumerate(polar)]
print("sum R:{}".format(sum(polar)))
...
base_angle=np.linspace(25,40,160)
for i in base_angle:
    grating=Triangular(4*1e-6,i,1)
    a,a_diff=grating.profile()
    Constant['a']=a
    ...
    R_effi.append(etaR_TE[0])#只取-6级光
...
plt.plot(base_angle,R_effi,label='Reflection')
plt.legend()
plt.xlabel("三角光栅底角变化")
plt.ylabel("-6级光衍射效率")
plt.title("TM偏振下的三角光栅底角参数扫描")
plt.show()
max_ind=np.where(R_effi==max(R_effi))
print("最大衍射效率对应的底角为:"+str(base_angle[max_ind]))
print("最大的衍射效率为:"+str(max(R_effi)))
# print(polar)

...
def Bessel_fourier_coeffi_sin(beta,depth,k0,m1,m2):
    '''
    ...
    m=np.arange(m1,m2+1)
    z=beta*k0*depth/2
    coeffi=np.exp(1j*z)*jv(m,z)
    return coeffi
    
def Bessel_fourier_coeffi_cos(beta,k0,m1,m2,Constant):
    '''
    计算exp(j*beta*a'(x))的fourier系数
    a'(x)=depth*K/2*cos(Kx)
    '''
    m=np.arange(m1,m2+1)
    z=beta*Constant['depth']*np.pi*k0/Constant['period']
    coeffi=(1j)**m*jv(m,z)
    return coeffi
...
def GenerateFFieldsChand(a,b0,nDim,k0,d,m1,m2,real_Ray2_idx,real_Ray1_idx,SB1,SB2,Constant):
    ...
    Fm0=Bessel_fourier_coeffi_sin(b0,Constant['depth'],k0,m1,m2)
    ...
    if len(real_Ray1_idx)!=0:
        ...
        for M in range(min_ray1,max_ray1+1):
            ...
            beta=SB1[M-m1]
            Fmn0=Bessel_fourier_coeffi_sin(beta,Constant['depth'],k0,-nDim,nDim)
    ...
    if len(real_Ray2_idx)!=0:
        ...
        for k in range(min_ray2,max_ray2+1):
            ...
            beta=SB2[k-m1]
            Fmk0=Bessel_fourier_coeffi_sin(beta,Constant['depth'],k0,-nDim,nDim)
            q0=len(Fmk0)//2
            for m in range(m1,m2+1):
                Fmk[m-m1,k-min_ray2]=Fmk0[q0+m-k]
    return Fm0,Fmk,Fmn

def GenerateGFieldsChand(b0,a_mat,SB1,SB2,real_Ray1_idx,real_Ray2_idx,\
    m1,m2,nDim,imag_eig1p,imag_eig2n,Fmn,Fmk,Fm0,Fmq,Fmr,A,eps1,eps2,Constant):
...
def Compute(n1,n2,polar,Constant):
    ...
    ###对a_diff_vec采用Bessel展开
    a_diff_vec=F_series_gen(a_diff,nDim)
    # a_diff_vec=Bessel_fourier_coeffi_cos(1,k0,-nDim,nDim,Constant)
    ##观察傅里叶频谱分量
    ...
    #Assemble G-matrices
    Gmn,Gmk,Gm0,Gmq,Gmr=GenerateGFieldsChand(b0,a_mat,SB1,SB2,real_Ray1_idx,real_Ray2_idx,m1,m2,nDim,imag_eig1p,
    imag_eig2n,Fmn,Fmk,Fm0,Fmq,Fmr,A,Constant['eps1'],Constant['eps2'],Constant)
...
def SortEigenvalueChand(V1,rho1,V2,rho2,tol,Constant):
    ...
    cand1_idx=np.where(np.real(rho1)>0)[0]
    cand1=rho1[cand1_idx]

    idx_sort1=np.argsort(np.abs(np.imag(cand1)))
    selected_idx1=cand1_idx[idx_sort1[:nDim]]

    prop_idx1=selected_idx1[:n_prop1]
    evan_idx1=selected_idx1[n_prop1:]

    real_eig1p=rho1[prop_idx1]
    imag_eig1p=rho1[evan_idx1]

    real_vec1p=V1[:nDim,prop_idx1]
    imag_vec1p=V1[:nDim,evan_idx1]
    ###传播模,按实部降序
    idx_prop1=np.argsort(-np.real(real_eig1p))
    real_eig1p=real_eig1p[idx_prop1]
    real_vec1p=real_vec1p[:,idx_prop1]
    ###倏逝模,按虚部升序
    idx_evan1=np.argsort(np.abs(np.imag(imag_eig1p)))
    imag_eig1p=imag_eig1p[idx_evan1]
    imag_vec1p=imag_vec1p[:,idx_evan1]
    #=========处理D2区域================
    n_prop2=len(Constant['real_Ray2_idx'])

    cand2_idx=np.where(np.real(rho2)<0)[0]
    cand2=rho2[cand2_idx]

    idx_sort2=np.argsort(np.abs(np.imag(cand2)))
    selected_idx2=cand2_idx[idx_sort2[:nDim]]

    prop_idx2=selected_idx2[:n_prop2]
    evan_idx2=selected_idx2[n_prop2:]

    real_eig2n=rho2[prop_idx2]
    imag_eig2n=rho2[evan_idx2]

    real_vec2n=V2[:nDim,prop_idx2]
    imag_vec2n=V2[:nDim,evan_idx2]
    ...
    imag_vec2n=imag_vec2n[:,idx_evan2]
...
depth=Constant['wavelength']
grating=Sinusoidal(2*Constant['wavelength'],1,depth)

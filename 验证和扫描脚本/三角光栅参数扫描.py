...
##########以下为光栅常数设定##############
grating=Triangular(4*1e-6,40,0.3)
...
if TE<1e-10:
    TE=0
file_path='C:/Users/123/Desktop/三角光栅6级光精细参数扫描.xlsx'
...
save_interval=10
counter=0
for f in np.arange(0.8,1.1,0.01):
    for baseangle in np.arange(35,40.01,0.01):
        grating=Triangular(4*1e-6,baseangle,f)
        a=grating.profile()
        Constant['a']=Constant['k0']*a(x)#光栅表面轮廓函数
        a_diff=np.gradient(Constant['a'],Constant['period']*Constant['k0']/2**10)

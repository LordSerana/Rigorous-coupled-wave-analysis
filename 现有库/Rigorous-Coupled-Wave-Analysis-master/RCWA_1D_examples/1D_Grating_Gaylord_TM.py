...
def grating_fourier_harmonics(order, fill_factor, n_ridge, n_groove):
    ...
    #该函数用于计算光栅区域的介电常数，且是用于Rytov条件展开的。
...
def fourier_reconstruction_general(x, period, num_ord, coefs):
    '''
    overloading doesn't work in python...fun fact, since it is dynamically typed (vs statically typed)
    :param x:
    :param period:
    :param num_ord:
    :param coefs:
...
def grating_fft(eps_r):
    #应该是用于光栅介电系数为常数的情况的展开
    assert len(eps_r.shape) == 2
    assert eps_r.shape[1] == 1
    #eps_r: discrete 1D grid of the epsilon profile of the structure，翻译：光栅1维的离散的介电常数数组
    fourier_comp = np.fft.fftshift(np.fft.fft(eps_r, axis = 0)/eps_r.shape[0])#除以采样点数，以归一化
...
# e0 = 8.854e-12
# mu0 = 4*np.pi*1e-8
#写在这干什么用的？L0,e0,mu0都是没用的参数
fill_factor = 0.3 # 50% of the unit cell is the ridge material，占空比
...
#num_ord为介电常数展开的傅里叶谐波数,或者说是倏逝波的级次
PQ = 2*num_ord+1
indices = np.arange(-num_ord, num_ord+1)#长度为2*num_ord+1

n_ridge = 3.48 #3.48;              # ridge
n_groove = 3.48               # groove (unit-less)
lattice_constant = 0.7#光栅脊部的长度为0.7
...
eps_r[0:border] = n_ridge**2#光栅脊部部分的介电常数区域
...
#填充耦合波系数矩阵
for prow in range(2 * num_ord + 1):#行
    # first term locates z plane, 2nd locates y coumn, prow locates x
    #row_index = p_index[prow]
    for pcol in range(2 * num_ord + 1):#列
        pfft = p_index[prow] - p_index[pcol]#i-p位置的索引。0····-2n,2n···0
